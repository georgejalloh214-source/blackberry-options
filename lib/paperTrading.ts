...
export async function openTrade(input: OpenTradeInput): Promise<PaperPosition> {
  ...
  const marginRequired = isShort
    ? round(input.optionContract.strike * 100 * input.quantity) // cash-secured
...
async function refreshPosition(pos: PaperPosition): Promise<PaperPosition> {
  ...
  if (!contract) return pos; // expiry passed or strike out of window — keep last mark
...
/** 0-100: |delta| is ~ITM probability; near expiry + ITM => risk spikes. */
